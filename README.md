...
<div class="columns-2">
  ...
  - **demo:** Demo Rmd script running the two Stan models for normalization and extraction of fitness scores. A PDF with the expected figures is provided.
  ...
  - **data:** Essential data files required for recreating Figure 1 and 2 in [^f1]. Additionally contains two demo datasets.
  ...
  - **man:** Contains a PDF file which describes the statistical model implemented in the two Stan model files.
...
For smaller (demo) datasets, consider using **cmdstanr**. Install the R package cmdstanr **and** cmdstan following the installation guide (example workflow in *example_cmdstanr.Rmd*):
