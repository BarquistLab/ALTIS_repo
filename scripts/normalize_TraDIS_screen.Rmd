...
```{r}
sample_corr <- round(cor(logcounts), 1)
cond_rm11 <- cond %>% subset(exp_id<4 | exp_id>40) %>% arrange(exp_id, condition)
sample_corr <- sample_corr[cond_rm11$sample, cond_rm11$sample]
...
p_corr_zoom <- p_corr_zoom +
  scale_x_discrete(name="Library", breaks=paste0("Lib", c(1,2,3,4,41,42,43), "_INPUT"), labels=c(1,2,3, "",41,42,43)) +
  scale_y_discrete(name="Library", breaks=paste0("Lib", c(1,2,3,4,41,42,43), "_INPUT"), labels=c(1,2,3, "",41,42,43)) +
  ...
  theme(rect=element_rect(fill="transparent"),
        ...
        panel.grid=element_blank(), axis.text=element_text(color = "black"))
print(p_corr_zoom)
cairo_pdf(paste(res_dir, "shigella_corr_mat_1-10.pdf", sep=""), width = 1.3, height = 1.5)
print(p_corr_zoom)
dev.off()
...
```{r}
dfit_org <- fread(paste0(res_dir, "dfit_all_org.csv"), quote=F, data.table=F)
dfit_new <- fread(paste0(res_dir, "dfit_all.csv"), quote=F, data.table=F) %>% mutate(it_g_org=dfit_org$it_g[match(locus_tag, dfit_org$locus_tag)])
dfit_pseudo_org <- fread(paste0(res_dir, "dfit_pseudo_org.csv"), quote=F, data.table=F)
dfit_pseudo <- fread(paste0(res_dir, "dfit_pseudo.csv"), quote=F, data.table=F)

counts_org <- fread(paste0(res_dir, "counts_all_org.csv"), quote=F, data.table=F)
counts_new <- fread(paste0(res_dir, "counts_all.csv"), quote=F, data.table=F)

subset(dfit_new, it_g/it_g_org!=1) %>% head()
```
...
lab <- "all_Bnorm"
dfit_NZ <- export_cmdstan(dfit_file, counts_file, res_dir, lab, norm_fac=norm_fac, b=b, a_in=a_in, organism="Shigella")
